...
namespace BTL_nhom2_demo
{
    public partial class CaLam : Form
    {
        ...
        private void data_GV_caLam_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            txt_maCa.Enabled = false;
            DataGridViewRow row = new DataGridViewRow();
            row = data_GV_caLam.Rows[e.RowIndex];
            txt_maCa.Text = Convert.ToString(row.Cells["ma_ca"].Value);
            txt_tenCa.Text = Convert.ToString(row.Cells["ten_ca"].Value);
        }
    }
}
